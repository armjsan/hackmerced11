...
from database import (
    ...
    get_user_by_id, log_event,
)
from auth import (
    create_user, verify_password_a, verify_password_b,
    verify_token_c, reset_user,
)
...
def api_verify_a():
    ...
    if result.get('success'):
        ...
        return jsonify({'success': True, 'requires_b': True})
...
def api_admin_users():
    db = get_db()
    rows = db.execute(
        "SELECT id, username, email, is_locked, failed_a_count, is_admin, created_at FROM users ORDER BY created_at DESC"
    ).fetchall()
...
def api_admin_create_user():
    ...
    if not username or not email or not password_a or not password_b:
        return jsonify({'success': False, 'error': 'All fields are required'}), 400
    ...
    try:
        token_c = create_user(username, email, password_a, password_b)
        return jsonify({'success': True, 'token_c': token_c})
    except Exception as e:
...
def api_admin_update_settings():
    data = request.get_json(silent=True) or {}
    allowed_keys = {
        'max_failed_a_attempts', 'smtp_server', 'smtp_port',
